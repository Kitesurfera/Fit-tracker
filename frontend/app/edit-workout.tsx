    ...
    const fetchWorkoutData = async () => {
      ...
      try {
        ...
        if (w) {
          ...
          // Cargar microciclos
          if (w.athlete_id) {
            const tree = await api.getPeriodizationTree(w.athlete_id);
        ...
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.section}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <TouchableOpacity 
                style={[styles.microChip, { borderColor: colors.border }, selectedMicroId === null && { backgroundColor: colors.primary, borderColor: colors.primary }]}
              ...
              {microciclosDisponibles.map(m => (
                <TouchableOpacity 
                  key={m.id}
                  style={[styles.microChip, { borderColor: colors.border }, selectedMicroId === m.id && { backgroundColor: m.color || colors.primary, borderColor: m.color || colors.primary }]}
                  onPress={() => setSelectedMicroId(m.id)}
                >
          ...
          {workoutType === 'traditional' ? (
              ...
              {exercises.map((ex, i) => (
                <View key={ex._key} style={[styles.exerciseCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                  <View style={styles.exerciseHeader}>
                    ...
                    <TouchableOpacity onPress={() => removeExercise(i)} style={styles.removeExBtn}><Ionicons name="trash-outline" size={18} color={colors.error || '#EF4444'} /></TouchableOpacity>
                  </View>
            ...
            <View style={styles.section}>
              {hiitBlocks.map((block, bIndex) => (
                <View key={block._key} style={[styles.hiitBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                  <View style={[styles.hiitHeader, { borderBottomColor: colors.border }]}>
                    ...
                    <TouchableOpacity onPress={() => removeHiitBlock(bIndex)}><Ionicons name="trash-outline" size={20} color={colors.error || '#EF4444'} /></TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.hiitExList}>
                    {block.exercises.map((ex: any, eIndex: number) => (
                      <View key={ex._key} style={styles.hiitExRow}>
                        ...
                        <TextInput style={[styles.hiitExInput, { flex: 1, color: colors.textPrimary, borderColor: colors.border }]} value={ex.duration_reps} onChangeText={v => updateHiitExercise(bIndex, eIndex, 'duration_reps', v)} placeholder="40s / 15 reps" />
              ...
              <TouchableOpacity onPress={addHiitBlock} style={[styles.addExBtnBig, { borderColor: colors.error || '#EF4444', borderStyle: 'dashed' }]}><Ionicons name="add" size={20} color={colors.error || '#EF4444'} /><Text style={{ color: colors.error || '#EF4444', fontWeight: '700' }}>Añadir Bloque HIIT</Text></TouchableOpacity>
            </View>
          )}
