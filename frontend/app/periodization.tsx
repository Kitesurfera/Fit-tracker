...
export default function PeriodizationScreen() {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={{ paddingBottom: 50 }}>
        ...
        <View style={styles.topActions}>
          <TouchableOpacity style={[styles.mainBtn, { backgroundColor: colors.primary }]} onPress={openNewMacro}>
            ...
            <Text style={styles.mainBtnText}>CREAR MACROCICLO</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.mainBtn, { backgroundColor: colors.surfaceHighlight, borderWidth: 1, borderColor: colors.border }]} 
            onPress={() => router.push({ pathname: '/add-workout', params: { athlete_id: params.athlete_id, name: params.name } })}
          >
            <Ionicons name="barbell" size={20} color={colors.primary} />
            <Text style={[styles.mainBtnText, { color: colors.textPrimary }]}>ENTRENO SUELTO</Text>
          </TouchableOpacity>
        </View>
                  ...
                  <View key={micro.id} style={[styles.microCard, { borderLeftColor: micro.color }]}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <View style={{ alignItems: 'flex-end', gap: 6 }}>
                        ...
                        {/* BOTÓN PARA AÑADIR ENTRENO DENTRO DEL MICRO */}
                        <TouchableOpacity 
      ...
      {/* --- MODALES OMITIDOS AQUÍ PARA BREVEDAD (MANTIENEN EL MISMO CÓDIGO QUE EN EL PASO ANTERIOR) --- */}
      {/* Pega aquí los Modales del código anterior (macroModalVisible y microModalVisible) */}
...
const styles = StyleSheet.create({
  ...
  mainBtnText: { color: '#FFF', fontSize: 12, fontWeight: '800', letterSpacing: 0.5 },
  ...
  // --- RESTO DE ESTILOS DE LOS MODALES ---
});
