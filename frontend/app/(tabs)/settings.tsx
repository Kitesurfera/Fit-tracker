...
export default function SettingsScreen() {
  const { user, logout, updateUser } = useAuth(); // Usamos updateUser para refrescar el estado global
  ...
  // Estados para preferencias de unidades
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (updateUser) updateUser({ name }); // Sincronizamos con el AuthContext
      setEditing(false);
      Alert.alert("¡Hecho!", "Tu nombre de perfil ha sido actualizado.");
    } catch (e) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 3: PREFERENCIAS DE UNIDADES (Recuperado) */}
        ...
        <Text style={styles.versionText}>Fit Tracker Pro v2.2 • Built for Performance</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
