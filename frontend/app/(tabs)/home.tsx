...
export default function HomeScreen() {
  ...
  const handleSaveAthlete = async () => {
    ...
    try { if (editingAthleteId) { if (api.updateAthlete) await api.updateAthlete(editingAthleteId, athleteForm); } else { if (api.createAthlete) await api.createAthlete(athleteForm); } setShowAthleteModal(false); loadData(); } catch (e) { if (Platform.OS !== 'web') Alert.alert("Error", "No se pudo guardar la información."); }
  };
      ...
      <View 
        ...
        style={[
          ...
          { 
            ...
            opacity: item.completed && !isExpanded ? 0.7 : 1,
      ...
      >
        ...
        {isExpanded && (
          <View style={{ marginTop: 15, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 15 }}>
                ...
                return (
                  <View key={idx} style={[styles.microWorkoutCard, { borderColor: colors.border, marginBottom: 8 }]}>
                    <TouchableOpacity 
                      ...
                      onPress={() => setExpandedExerciseId(isBlockExpanded ? null : blockKey)}
                    >
                      <Ionicons name={ex.is_hiit_block ? "flash" : "barbell-outline"} size={16} color={colors.primary} />
                      <View style={{ flex: 1, marginLeft: 10 }}>
                        <Text style={{ color: colors.textPrimary, fontWeight: '700', fontSize: 13 }}>{ex.name}</Text>
                        {!ex.is_hiit_block && <Text style={{ color: colors.textSecondary, fontSize: 11 }}>{ex.sets} series x {ex.reps} reps</Text>}
                      </View>
                      ...
                      <View style={[styles.microWorkoutExercises, { borderTopColor: colors.border, backgroundColor: colors.surface }]}>
                        {ex.is_hiit_block ? (
                          ex.hiit_exercises?.map((he: any, j: number) => (
                            <Text key={j} style={{ color: colors.textSecondary, fontSize: 12, marginBottom: 4 }}>• {he.name} <Text style={{fontWeight: '700', color: colors.textPrimary}}>({he.duration_reps})</Text></Text>
                          ))
                        ) : (
                          <View>
                            <Text style={{ color: colors.textSecondary, fontSize: 12 }}>Objetivo: <Text style={{fontWeight: '700', color: colors.textPrimary}}>{ex.sets} series de {ex.reps} {ex.weight ? `con ${ex.weight}` : ''}</Text></Text>
                            {ex.rest && <Text style={{ color: colors.textSecondary, fontSize: 12, marginTop: 2 }}>Descanso: {ex.rest}</Text>}
                            {ex.exercise_notes && <Text style={{ color: colors.textSecondary, fontSize: 12, marginTop: 4, fontStyle: 'italic' }}>Nota: {ex.exercise_notes}</Text>}
                          </View>
                        )}
                      </View>
                    )}
            ...
            <TouchableOpacity
              style={{backgroundColor: item.completed ? colors.surfaceHighlight : colors.primary, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 16, borderRadius: 14, marginTop: 10, gap: 8}}
              onPress={() => router.push(`/training-mode?workoutId=${item.id}`)}
              activeOpacity={0.8}
            >
              <Ionicons name={item.completed ? "stats-chart" : "play"} size={20} color={item.completed ? colors.textPrimary : "#FFF"} />
              <Text style={{color: item.completed ? colors.textPrimary : '#FFF', fontWeight: '800', fontSize: 14}}>
                {item.completed ? "VER RESUMEN Y FEEDBACK" : "INICIAR ENTRENAMIENTO"}
              </Text>
            </TouchableOpacity>
          </View>
        )}
          ...
          <TouchableOpacity activeOpacity={1} style={[styles.modalContentInfo, { backgroundColor: colors.surface }]}>
            {!!viewMicroInfo && (
              <View style={{ alignItems: 'center', width: '100%', flex: 1 }}>
                ...
                <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>MACROCICLO:</Text><Text style={[styles.infoTitleMacro, { color: colors.textPrimary }]}>{viewMicroInfo.macroNombre}</Text>
                <View style={[styles.divider, { backgroundColor: colors.border }]} /><Text style={[styles.infoLabel, { color: colors.textSecondary }]}>FASE ACTUAL:</Text><Text style={[styles.infoTitleMicro, { color: colors.textPrimary }]}>{viewMicroInfo.nombre}</Text>
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 15 }}><View style={[styles.microTypeBadgeBig, { backgroundColor: (viewMicroInfo.color || colors.primary) }]}><Text style={{ color: '#FFF', fontSize: 12, fontWeight: '900' }}>{viewMicroInfo.tipo}</Text></View></View>
                <View style={{ width: '100%', marginTop: 25, flexShrink: 1 }}><Text style={[styles.infoLabel, { color: colors.textSecondary, marginBottom: 10, textAlign: 'left' }]}>SESIONES ({microWorkouts.length})</Text>
